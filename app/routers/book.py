...
def get_books(db: Session = Depends(get_db), current_user: schemas.User = Security(oauth2.get_current_user, scopes=["books:read"])):
    results = books_crud.get_books(db)
    return results


@router.get("/{id}", response_model=schemas.Book)
def get_book(id: int, db: Session = Depends(get_db), current_user: schemas.User = Security(oauth2.get_current_user, scopes=["books:read"])):
...
def create_book(book: schemas.BookCreate, db: Session = Depends(get_db), current_user: schemas.User = Depends(oauth2.get_current_user)):
...
def update_book(id: int, updated_book: schemas.BookCreate, db: Session = Depends(get_db), current_user: schemas.User = Depends(oauth2.get_current_user)):
    if current_user.role != utils.Roles.ADMIN:
         raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail=f"permission denied")
...
def borrow(borrow_data: schemas.Borrow, db: Session = Depends(get_db), current_user: schemas.User = Security(oauth2.get_current_user, scopes=["books:borrow"])):
...
def return_book(id: int, db: Session = Depends(get_db), current_user: schemas.User = Security(oauth2.get_current_user, scopes=["books:return"])):
    book = books_crud.get_book_by_id(db, id)
    if not book:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"book with id {id} was not found")
